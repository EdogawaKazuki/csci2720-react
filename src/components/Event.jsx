...
class Event extends React.Component{
    ...
    getCommentList(page){
        fetch(`http://localhost:9000/api/comments/eventId/${this.props.currentEvent.event_id}/page/${page}`)
            .then(res => res.json())
            .then((result) => {
                this.setState({
                    comment: result,
    ...
    deleteComment(commentId){
        ...
        //console.log(requestOptions)
        fetch(`http://localhost:9000/api/comments/${commentId}`, {method: 'DELETE'})
            .then(res => res.json())
            .then(result => {
                //console.log(result)
                this.getCommentList(1);
